...
async function processSingleClipJob(jobId) {
    ...
    switch (action) {
        // --- CASO NOVO: EXPORTAÇÃO (CORRIGE O LOG DE ERRO) ---
        case 'export': 
            // Renderização final de alta qualidade (H.264 High Profile)
            args = [
                '-i', videoFile.path,
                '-c:v', 'libx264',
                '-preset', 'medium',  // Mais lento que ultrafast, mas melhor qualidade/tamanho
                '-crf', '23',         // Qualidade visual padrão
                '-pix_fmt', 'yuv420p',
                '-c:a', 'aac', '-b:a', '192k',
        ...
        case 'ai-dubbing':
            job.status = 'failed'; job.error = "AI Dubbing requer configuração de chaves de API.";
            return; 
...
// 1. ROTA NOVA: Exportação Final (CORREÇÃO DO SEU ERRO)
app.post('/api/export/start', uploadAny, (req, res) => {
    // Essa é a rota que estava dando 404
    ...
    // Inicia o processo com a flag 'export' que criamos no switch
    processSingleClipJob(jobId);
    
    res.status(202).json({ jobId });
});

// 2. Rota de Extração Simples (MP3)
