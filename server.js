...
const transitionBuilder = {
    buildConcatFilter: (inputs) => {
        ...
        inputs.forEach((_, i) => {
            ...
            // Using -1:-1 for pad automatically centers the image and handles odd dimensions safely.
...
async function handleExport(job, createFFmpegJob) {
    let projectClips = [];
    if (job.params && job.params.projectState) {
        try {
            const state = JSON.parse(job.params.projectState);
            projectClips = state.clips || [];
        } catch (e) {
            console.error("Export: Failed to parse project state", e);
        }
    }

    const fileMap = {};
    if (job.files) {
        job.files.forEach(f => fileMap[f.originalname] = f);
    }

    const orderedInputs = [];
    const addInput = (file, duration, isImage) => {
        orderedInputs.push({ file, duration, isImage });
    };

    if (projectClips.length > 0) {
        // Filter visual clips (video/image on video/camada/text tracks)
        // Ignore audio tracks to prevent stream mismatch errors in concat
        const visualClips = projectClips
            .filter(c => (c.track === 'video' || c.track === 'camada' || c.track === 'text') && (c.type === 'video' || c.type === 'image'))
            .sort((a, b) => a.start - b.start);

        visualClips.forEach(c => {
            const file = fileMap[c.fileName];
            if (file) {
                addInput(file, c.duration, c.type === 'image');
            }
        });
    } 
    
    // Fallback if no valid project clips found (e.g. simple file list export)
    if (orderedInputs.length === 0 && job.files) {
        job.files.forEach(f => {
            if (f.mimetype.startsWith('video/') || f.mimetype.startsWith('image/')) {
                addInput(f, 5, f.mimetype.startsWith('image/'));
            }
        });
    }
    ...
    if (orderedInputs.length === 0) {
        job.status = 'failed';
        job.error = "No visual clips found to export. Please add video or images.";
        return;
    }
    ...
    orderedInputs.forEach(item => {
        if (item.isImage) {
            args.push('-loop', '1', '-t', String(item.duration)); 
        }
        args.push('-i', item.file.path);
        expectedDuration += item.duration;
    });

    const { filterComplex, outputMap } = transitionBuilder.buildConcatFilter(orderedInputs);
    ...
    if (expectedDuration <= 0) expectedDuration = 30; // Fallback duration
