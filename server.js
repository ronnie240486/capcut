...
const simulateAiProcess = (req, res, friendlyName) => {
  ...
  setTimeout(() => {
    console.log(`[AI Job Simulado] ${friendlyName} concluído. A devolver o ficheiro original.`);
    res.sendFile(path.resolve(inputPath), (err) => {
      if (err) console.error(`Erro ao enviar ficheiro simulado de ${friendlyName}:`, err);
      fs.unlink(inputPath, (unlinkErr) => unlinkErr && console.error("Falha ao apagar ficheiro:", unlinkErr));
    });
  }, 3000);
};

// --- Rotas ---
app.get('/', (req, res) => res.status(200).json({ message: 'Servidor ProEdit ativo.' }));
app.post('/api/projects', (req, res) => {
  console.log('Recebido novo projeto:', req.body.name);
...
app.get('/api/export/download/:jobId', (req, res) => {
  ...
  if (!job || job.status !== 'completed' || !job.outputPath)
    return res.status(404).json({ message: 'Ficheiro não encontrado ou tarefa não concluída.' });
...
function processExportJob(jobId) {
  ...
  try {
    ...
    videoClips.forEach((clip, idx) => {
      ...
      // Corrige pixel format para evitar warning
      filterChains.push(`[${inputIndex}:v]scale=1280:720,format=yuv420p,setsar=1,setpts=PTS-STARTPTS[v${idx}]`);
    });
    ...
    commandArgs.push(audioClips.length > 0 ? "-map" : "-map", audioClips.length > 0 ? "[outa]" : `${files.length}:a`);
    commandArgs.push("-c:v", "libx264", "-c:a", "aac", "-preset", "veryfast", "-pix_fmt", "yuv420p", "-r", "30", "-shortest", "-progress", "pipe:1", "-t", totalDuration, outputPath);
