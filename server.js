...
function processExportJob(jobId) {
    ...
    try {
        ...
        videoAndLayerClips.forEach((clip, vIdx) => {
            ...
            // CRITICAL FIX: Use -1:-1 for pad to allow FFmpeg to auto-center. 
            // Previous explicit math (ow-iw)/2 caused crashes with odd numbers.
            const preFilter = `[${inputIndex}:v]${clipSpecificFilters.length > 0 ? clipSpecificFilters.join(',')+',' : ''}scale=${width}:${height}:force_original_aspect_ratio=decrease,pad=${width}:${height}:-1:-1:color=black,setsar=1`;
        ...
        // Always create a silent audio track if no clips exist, to ensure valid MP4 structure
        const silentAudioIndex = files.length; // Index for the generated silence
        if (audioClips.length === 0) {
        ...
        if (hasAudioOutput) {
            ...
            // Map the silent audio source generated by anullsrc
            commandArgs.push("-map", `${silentAudioIndex}:a`);
        }
        
        // Add -shortest so looped images don't cause infinite encoding if audio is shorter (though -t handles total duration)
        // Using -t ensures we cut exactly at totalDuration
...
async function processSingleClipJob(jobId) {
    ...
    let videoFile = (job.files && job.files.video && job.files.video[0]) ? job.files.video[0] : null;
    
    // Fallback: If voice-clone or auto-ducking and no video, check audio
    if (!videoFile && (action === 'voice-clone' || action === 'auto-ducking-real')) {
        if (job.files && job.files.audio && job.files.audio[0]) {
