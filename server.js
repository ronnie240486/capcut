...
import { handleExportVideo } from './videoengine/exportVideo.js';
import filterBuilder from './videoengine/filterBuilder.js';
    ...
    return new Promise((resolve) => {
        ...
        ffprobe.on('close', (code) => {
            try {
                ...
                resolve({
                    hasAudio: json.streams && json.streams.some(s => s.codec_type === 'audio'),
                    hasVideo: json.streams && json.streams.some(s => s.codec_type === 'video')
                });
            } catch (e) {
...
app.post('/api/process/start/:action', uploadAny, async (req, res) => {
    ...
    setTimeout(() => {
        ...
        // If it's an audio file or an audio action without video, use .mp3
        ...
        if (action.includes('extract-audio')) {
            ...
            // Critical fix: if outputting mp3, ensure no video stream is mapped (prevents "attached picture" error)
            if (ext === '.mp3') {
                args = args.filter(a => a !== '0:v' && a !== '-map'); 
                // Re-add -map [a] if it was filtered out or ensure it's there
