...
async function startServer() {
                ...
                try {
                    ...
                    if (ep.version === 'v2') {
                        ...
                        if (resolvedType === 'sfx') {
                            ...
                            const rawLangInput = req.body.lang || resolvedLang || 'pt-br';
                            const rawLang = String(rawLangInput).toLowerCase().trim();
                            
                            // Tentar encontrar linguagem suportada pelo modelo se disponível
                            let finalLang = rawLang;
                            const modelInfo = availableModels.find((m: any) => m.slug === mappedModel);
                            
                            const langMap: Record<string, string> = {
                                'portuguese': 'pt', 'português': 'pt', 'pt-br': 'pt-br', 'pt': 'pt',
                                'english': 'en', 'en-us': 'en-us', 'en-gb': 'en-gb', 'en': 'en',
                                'spanish': 'es', 'español': 'es', 'es': 'es',
                                'french': 'fr', 'fr-fr': 'fr-fr', 'fr': 'fr',
                                'italian': 'it', 'it': 'it',
                                'hindi': 'hi', 'hi': 'hi',
                                'japanese': 'ja', 'ja': 'ja',
                                'chinese': 'zh', 'zh': 'zh',
                            ...
                            if (modelInfo?.languages && Array.isArray(modelInfo.languages)) {
                                const supportedLangs = modelInfo.languages.map((l: any) => (l.code || l.lang || l.value || l.id || '').toLowerCase());
                                console.log(`[Deapi Audio] Model ${mappedModel} supported langs: ${supportedLangs.join(', ')}`);
                                
                                const normalizedRaw = langMap[rawLang] || rawLang;

                                // 1. Match exato com input do usuário (ex: pt-br)
                                if (supportedLangs.includes(rawLang)) {
                                    finalLang = rawLang;
                                } 
                                // 2. Match com mapeamento (ex: pt)
                                else if (supportedLangs.includes(normalizedRaw)) {
                                    finalLang = normalizedRaw;
                                }
                                // 3. Match parcial (ex: se input é pt-br e suporta pt)
                                else {
                                    const bestMatch = supportedLangs.find((l: string) => 
                                        l === rawLang.split('-')[0] || 
                                        l.startsWith(rawLang.split('-')[0]) ||
                                        rawLang.startsWith(l)
                                    );
                                    
                                    if (bestMatch) {
                                        finalLang = bestMatch;
                                    } else {
                                        // 4. Último recurso: usar primeiro item da lista ou manter raw se nada bater
                                        finalLang = supportedLangs[0] || rawLang;
                                    }
                                }
                            } else {
                                // Se não temos info do modelo, usamos o mapeamento manual como fallback seguro
                                finalLang = langMap[rawLang] || rawLang;
                            }

                            console.log(`[Deapi Audio] v2 Language resolution: input="${rawLangInput}" (raw="${rawLang}") model=${mappedModel} -> finalLang="${finalLang}"`);
                        ...
                        if (resolvedType !== 'sfx') {
                            ...
                            if (modelInfo?.languages && Array.isArray(modelInfo.languages)) {
                                ...
                                if (supportedLangs.includes(rawLang)) {
                                    ...
                                    finalLang = bestMatch || supportedLangs[0] || rawLang;
                                }
                            } else {
                                finalLang = langMap[rawLang] || rawLang;
                            }
                            
                            console.log(`[Deapi Audio] v1 Language resolution: input="${rawLangInput}" model=${mappedModel} -> finalLang="${finalLang}"`);
                            
                            v1Payload.lang = finalLang;
                            ...
                            // ref_text
                            const finalRefText = ref_text || refText || req.body.refText || req.body.ref_text;
                            if (finalRefText) v1Payload.ref_text = finalRefText;
                        }
                        fetchOptions = {
                            method: 'POST',
                            headers: {
    ...
    app.post('/api/ai/generate-music', async (req: any, res: any) => {
        ...
        try {
            ...
            // NOTE: Recent Deapi updates for music models seem to enforce a minimum of 3.0
            // even if internal model limits (like ACE-Step) suggest lower.
            const maxGuidance = modelLimits.max_guidance_scale ?? modelLimits.max_guidance ?? 10;
            let minGuidance = modelLimits.min_guidance_scale ?? modelLimits.min_guidance ?? 3; // Default to 3 as baseline
            
            // If it's an ACE model, we might still want to allow lower IF the model limits say so,
            // but if we got a 422, it's safer to default to 3.
            if (mappedModel.toLowerCase().includes('ace')) {
                minGuidance = modelLimits.min_guidance_scale ?? modelLimits.min_guidance ?? 3.0;
            }

            // Sensible defaults if model limits are unknown
            const defaultTarget = mappedModel.toLowerCase().includes('ace') ? 3.0 : 5;
            const guidanceScale = Math.min(Math.max(defaultTarget, minGuidance), maxGuidance);

            const resolvedDuration = duration || 30;
            const finalGuidance = userGuidance ? Math.min(Math.max(Number(userGuidance), minGuidance), maxGuidance) : guidanceScale;

            console.log(`[Deapi Music] model=${mappedModel} duration=${resolvedDuration}s guidance_scale=${finalGuidance.toFixed(1)}`);
            ...
            for (const ep of ENDPOINTS) {
                ...
                try {
                    ...
                    if (ep.version === 'v2') {
                        ...
                        form.append('guidance_scale', String(finalGuidance));
